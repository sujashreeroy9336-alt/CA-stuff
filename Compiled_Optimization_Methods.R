...
#
# Model:
# y_i = theta1*x_i/(theta2 + x_i) + epsilon_i
# ============================================================
...
y <- c(0.91, 1.67, 2.14, 2.78, 3.02,
       3.51, 3.78, 4.12, 4.25, 4.53)

data <- data.frame(x, y)


# ------------------------------------------------------------
# 2. NONLINEAR LEAST SQUARES USING nls()
# ------------------------------------------------------------
...
theta1.hat <- coef(fit)[["theta1"]]
theta2.hat <- coef(fit)[["theta2"]]

cat("Estimated theta1 =", theta1.hat, "\n")
cat("Estimated theta2 =", theta2.hat, "\n")


# ------------------------------------------------------------
# 5. FITTED VALUES
#
# y_hat = theta1_hat*x/(theta2_hat + x)
# ------------------------------------------------------------

data$yhat <- predict(fit)
...
# ------------------------------------------------------------
# 8. MANUAL PROFILE SSE METHOD
#
# For a fixed theta2:
#
# theta1_hat(theta2) =
#
#       sum[x_i*y_i/(theta2+x_i)]
#       --------------------------------
#       sum[x_i^2/(theta2+x_i)^2]
#
# Then minimize SSE with respect to theta2.
# ------------------------------------------------------------

profile_sse <- function(theta2) {
  
  # Calculate theta1 for a given theta2
  theta1 <- sum(
    x * y / (theta2 + x)
  ) /
    sum(
...
# ------------------------------------------------------------
# 9. NUMERICALLY MINIMIZE PROFILE SSE
# ------------------------------------------------------------
...
# Estimated theta2 from numerical minimization
theta2.manual <- result$minimum


# ------------------------------------------------------------
# 10. ESTIMATE THETA1 USING THE ESTIMATED THETA2
# ------------------------------------------------------------
...
theta1.manual <- sum(
  ...
  sum(
    x^2 / (theta2.manual + x)^2
  )


cat("\nManual Profile SSE Estimates:\n")
cat("Estimated theta1 =", theta1.manual, "\n")
cat("Estimated theta2 =", theta2.manual, "\n")


# ------------------------------------------------------------
# 11. COMPARE nls() AND MANUAL RESULTS
# ------------------------------------------------------------

comparison <- data.frame(
  Parameter = c("theta1", "theta2"),
  nls_Estimate = c(theta1.hat, theta2.hat),
  Manual_Estimate = c(theta1.manual, theta2.manual)
)
...
# ------------------------------------------------------------
# 12. PLOT THE DATA AND NONLINEAR FIT
# ------------------------------------------------------------
...
cat("\n========================================\n")
cat("FINAL LSE RESULTS\n")
cat("========================================\n")

cat("theta1_hat =", theta1.hat, "\n")
cat("theta2_hat =", theta2.hat, "\n")
cat("SSE        =", SSE, "\n")
